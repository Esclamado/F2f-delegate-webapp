...
@Component({
  selector: 'app-my-schedule-virtual',
  templateUrl: './my-schedule-virtual.component.html',
  styleUrls: ['./my-schedule-virtual.component.scss'],
  providers: [DatePipe]
})
export class MyScheduleVirtualComponent implements OnInit , AfterViewInit , AfterContentInit{
  ...
  constructor(
    ...
    if(this.user && this.user.timezone && this.event.type == '2'){
      this.userTimezone = this.timezoneService.getTimezoneName(this.user.timezone);
    }
  ...
  getMySchedule(){
    ...
    let url = Urls.mapi_event_schedulevirtual;
    url += '?event_id=' + this.event.id;
    url += '&delegate_timezone=' + this.env.trim(this.user.timezone);
  ...
  viewMeetingDetail(timeslot){
    this.clockService = new ClockService();      
    this.clockService.setDateTime(this.event_delegate).startTime();   
    console.log(timeslot.data.meeting_schedule);
